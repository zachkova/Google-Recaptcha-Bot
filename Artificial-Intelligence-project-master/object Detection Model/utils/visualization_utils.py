...
def visualize_boxes_and_labels_on_image_array(
    ...
    for box, color in box_to_color_map.items():
        if scores[i] > min_score_thresh:
            print("the threshold is greater")
        ymin, xmin, ymax, xmax = box
        # Attempting to extract coordinate location of bounding boxes
        # relative to  the cropped image.
        height, width, channels = image.shape
        (left, right, top, bottom) = (xmin * width, xmax * width,
                                      ymin * height, ymax * height)
        bounding_box_coords = (left, right, top, bottom)
        text = box_to_display_str_map[box][0]

        box_dict.update({text: bounding_box_coords})
